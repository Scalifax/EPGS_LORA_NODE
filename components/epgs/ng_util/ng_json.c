...
struct _ng_json {
 ...
 struct _ng_json *ng_json_create(const char *json) {
 	struct _ng_json *p_json = (struct _ng_json*)ng_malloc(sizeof(struct _ng_json));
 	char *end_str;
 	
 	p_json->json_data_count = 0;
	p_json->json_all_data_size = 0;
	p_json->json_data = (char**) ng_malloc(sizeof(char**));
	 	
 	if(json){
 		int jsonLen = ng_strlen(json);
 		char *aux_token = (char*)ng_calloc(sizeof(char), jsonLen-1);
...
void ng_json_getJSon(NgJSon *ngJSON, char** jsonStr){
 	char* ng_json_result = (char*)ng_calloc((ngJSON->json_all_data_size + 2), sizeof(char));
		...
		char* token = strtok_r(aux_token, ":", &end_str);		
 		...
 		if(token)
 		{
 			...
 			if(comp==0) {			
				...
				value = (char*)ng_calloc(sizeof(char), len-1);
